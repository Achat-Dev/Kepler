...
namespace kepler {
    ...
    std::unique_ptr<AssignmentStatement> Parser::parse_assignment(const Token* identifier_token) {
        ...
        return std::make_unique<AssignmentStatement>(std::make_unique<VariableExpression>(identifier, identifier_token->source_location), std::move(value_expression), assignment_source_location);
    }
    ...
    std::unique_ptr<ForStatement> Parser::parse_for() {
        ...
        next_token(true); // eat '('
        if (current_token->type == TokenType::BracketClose) {
            diagnostic_sink.report(DiagnosticCode::UnexpectedToken, "Missing 'for' definition, expected at least '(<datatype> <identifier> : <end_value>)'", current_token->source_location);
            recover_for_definition_and_parse_body(for_source_location);
            return nullptr;
    ...
    std::unique_ptr<ForStatement> Parser::create_for_statement(const std::string& variable_identifier, const Token* variable_data_type_token, std::unique_ptr<Expression> start_value, std::unique_ptr<Expression> end_value, std::unique_ptr<Expression> step_value, const SourceLocation& for_source_location) {
        next_token(true); // eat ')'
        ...
        std::unique_ptr<VariableExpression> variable = std::make_unique<VariableExpression>(variable_identifier, variable_data_type_token->source_location);
        std::unique_ptr<AssignmentStatement> assignment_statement = std::make_unique<AssignmentStatement>(std::move(variable), std::move(start_value), variable_data_type_token->source_location);
        std::unique_ptr<VariableDefinitionStatement> variable_definition_statement = std::make_unique<VariableDefinitionStatement>(variable_data_type, variable_identifier, std::move(assignment_statement), variable_data_type_token->source_location);

        return std::make_unique<ForStatement>(std::move(variable_definition_statement), std::move(end_value), std::move(step_value), std::move(*body), for_source_location);
    }

    void Parser::recover_for_definition_and_parse_body(const SourceLocation& source_location) {
        recover(SynchronizationSet<TokenType::Newline, TokenType::End>{}, SynchronizationSet<TokenType::Newline>{});
        if (current_token->type == TokenType::End) {
            next_token(true); // eat 'end'
        } else {
    ...
    std::unique_ptr<ReturnStatement> Parser::parse_return() {
        ...
        switch (current_token->type) {
            ...
            case TokenType::DataType:
                ...
                diagnostic_sink.report(DiagnosticCode::UnexpectedToken, std::format("Unexpected token '{}' (data types can only be used for casting here)", *current_token), current_token->source_location);
                recover(SynchronizationSet<TokenType::Newline, TokenType::End>{}, SynchronizationSet<TokenType::Newline>{});
                return nullptr;
            case TokenType::End:
    ...
    std::unique_ptr<VariableDefinitionStatement> Parser::parse_variable_definition() {
        ...
        next_token(true); // eat data type
        if (current_token->type != TokenType::Identifier) {
            diagnostic_sink.report(DiagnosticCode::UnexpectedToken, "Expected identifier (data types can only be used for variable definitions here)", current_token->source_location);
            recover(SynchronizationSet<TokenType::Newline, TokenType::End>{}, SynchronizationSet<TokenType::Newline>{});
            return nullptr;
        }
        ...
        const std::string& identifier = std::get<std::string>(identifier_token->data);
        return std::make_unique<VariableDefinitionStatement>(data_type, identifier, std::move(assignment_statement), data_type_source_location);
    }
