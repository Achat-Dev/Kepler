...
namespace Kepler::Type {
    ...
    llvm::Type* Float64Type::get_llvm_type() const {
        return llvm::Type::getDoubleTy(Compiler::get().get_context());
    }

    llvm::Value* Float64Type::cast(llvm::Value* value, TypeToken to) const {
        switch (to) {
            case TypeToken::Bool: return Compiler::get().get_builder().CreateFCmpONE(value, llvm::ConstantFP::get(get_by_token(TypeToken::Float64), 0));
