...
namespace Kepler::Type {
    ...
    llvm::Type* Int64Type::get_llvm_type() const {
        return llvm::Type::getInt64Ty(Compiler::get().get_context());
    }

    llvm::Value* Int64Type::cast(llvm::Value* value, TypeToken to) const {
        switch (to) {
            case TypeToken::Bool: return Compiler::get().get_builder().CreateICmpNE(value, llvm::ConstantInt::get(get_by_token(TypeToken::Int64), 0));
            case TypeToken::Char:
                log(LogStyle::UNSUPPORTED, "[ Unpaid developer error ]", LogStyle::DEFAULT, ": casting from 'i64' to 'char' is not supported yet");
                return nullptr;
            case TypeToken::String: return cast_to_string(value, TypeToken::Int64);
            case TypeToken::Int8: return Compiler::get().get_builder().CreateTrunc(value, get_by_token(to));
            case TypeToken::Int16: return Compiler::get().get_builder().CreateTrunc(value, get_by_token(to));
            case TypeToken::Int32: return Compiler::get().get_builder().CreateTrunc(value, get_by_token(to));
