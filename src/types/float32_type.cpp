...
namespace Kepler::Type {
    ...
    llvm::Type* Float32Type::get_llvm_type() const {
        return llvm::Type::getFloatTy(Compiler::get().get_context());
    }

    llvm::Value* Float32Type::cast(llvm::Value* value, TypeToken to) const {
        switch (to) {
            case TypeToken::Bool: return Compiler::get().get_builder().CreateFCmpONE(value, llvm::ConstantFP::get(get_by_token(TypeToken::Float32), 0));
