...
namespace Kepler::Type {
    ...
    llvm::Type* Int8Type::get_llvm_type() const {
        return llvm::Type::getInt8Ty(Compiler::get().get_context());
    }

    llvm::Value* Int8Type::cast(llvm::Value* value, TypeToken to) const {
        switch (to) {
            case TypeToken::Bool: return Compiler::get().get_builder().CreateICmpNE(value, llvm::ConstantInt::get(get_by_token(TypeToken::Int8), 0));
            case TypeToken::Char:
            ...
            case TypeToken::Int16: return Compiler::get().get_builder().CreateSExt(value, get_by_token(to));
