...
namespace Kepler::Parser {
    ...
    static std::unique_ptr<AST::Expression> parse_identifier() {
        ...
        // This means we are using a variable (no parethesis after identifier)
    ...
    static std::shared_ptr<AST::Prototype> parse_prototype(Type::TypeToken type, std::string identifier) {
        if (identifier.empty()) {
            ...
            if (current_token != Lexer::Token::Identifier) {
        ...
        if (current_token != Lexer::Token::DataType && current_token != Lexer::Token::BracketClose) {
            log(LogStyle::ERROR, "[ Parsing error ]", LogStyle::DEFAULT, ": expected data type or ')' after '(' in prototype");
            return nullptr;
        }

        while (current_token == Lexer::Token::DataType) {
    ...
    static std::unique_ptr<AST::Expression> parse_if() {
        bool is_elseif = current_token == Lexer::Token::Parsing_Elseif;
        ...
        while (current_token != Lexer::Token::Elseif && current_token != Lexer::Token::Else && current_token != Lexer::Token::End) {
            auto expression = parse_expression();
            if (!expression) {
                if (is_elseif) {
    ...
    static std::unique_ptr<AST::Expression> parse_for() {
        ...
        read_next_token(); // eat identifier
        if (current_token != Lexer::Token::Colon) {
        ...
        if (current_token == Lexer::Token::BracketClose) {
            ...
            std::unique_ptr<AST::BinaryExpression> assignment = std::make_unique<AST::BinaryExpression>(Lexer::Token::Assignment, std::make_unique<AST::VariableExpression>(variable_name), std::move(start));
            std::unique_ptr<AST::VariableDefinitionExpression> variable = std::make_unique<AST::VariableDefinitionExpression>(type, variable_name, std::move(assignment));
    ...
    static std::unique_ptr<AST::Expression> parse_data_type() {
        ...
        // Parse local variable definition
        else if (current_token == Lexer::Token::Identifier) {
            std::string variable_name = Lexer::get_identifier();
            ...
            std::unique_ptr<AST::BinaryExpression> assignment_expression = std::make_unique<AST::BinaryExpression>(Lexer::Token::Assignment, std::make_unique<AST::VariableExpression>(variable_name), std::move(value));
    ...
    bool handle_data_type() {
        ...
        if (current_token == Lexer::Token::Identifier) {
            ...
            // Function definition
            else if (current_token == Lexer::Token::BracketOpen) {
